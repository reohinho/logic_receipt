...
/*
Report layout for Total Staff Transaction Report

Update History
---------------------
2013-02-21  Add total void amount 
...
public class DailyTransactionOfficerReportGenerate  {
  ArrayList data;
  ReportEnquiryData reData;
  
  public DailyTransactionOfficerReportGenerate(ArrayList data, ReportEnquiryData reData) {
    try {
      this.data = data;
      this.reData = reData;
    }catch(Exception e) {
	...
	public ByteArrayOutputStream getReportCode() { 
      ...
      if(data.size() == 0) {
		...
		double totalVoid = 0;		//2013-02-21 add total void amount
        double subTotalCash = 0;
        double subTotalCheque = 0;
		double subTotalVoid = 0;	//2013-02-21 add subtotal void amount
        double cashAmt = 0;
        double chequeAmt = 0;
		double voidAmt = 0;
        int border = 0;
        ...
        for(int i=0; i<data.size();i++) {
          ...
          cashAmt = Double.parseDouble(row[2]);
          chequeAmt = Double.parseDouble(row[3]);
          
          table.addCell(printStr(row[0],1,10,false,border,0));        
          table.addCell(printStr(row[1],1,10,false,border,0));                         
          table.addCell(printStr(cashAmt+"0",1,10,false,border,0));                
          table.addCell(printStr(chequeAmt+"0",1,10,false,border,0));                
          table.addCell(printStr(row[4],1,10,false,border,0));                
          table.addCell(printStr(row[5],1,10,false,border,0));                
          table.addCell(printStr(row[6],1,10,false,border,0));  
          table.addCell(printStr(row[7],1,10,false,border,0));                

          if(row[7]==null||!row[7].equals("Y"))
          {
  	          subTotalCash += cashAmt;
	          totalCash += cashAmt;
			  subTotalCheque += chequeAmt;
			  totalCheque += chequeAmt;	          	          
          }
		  else if ( row[7].equals("Y") )   //2013-02-21 if void flag = Y, add both cash and cheque
		  {
			  subTotalVoid += cashAmt+chequeAmt;
			  totalVoid += cashAmt+chequeAmt;
		  }
          
          if(officer)
          {
	          table.addCell(printStr("Staff "+row[6]+" Cash Collected: HK$ "+subTotalCash,16,12,false,PdfPCell.LEFT+PdfPCell.RIGHT,0));
	          table.addCell(printStr("Staff "+row[6]+" Cheque Collected: HK$ "+subTotalCheque,16,12,false,PdfPCell.LEFT+PdfPCell.RIGHT,0));
	          table.addCell(printStr("Staff "+row[6]+" Void Amount: HK$ "+subTotalVoid,16,12,false,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.BOTTOM,0));	  		//2013-02-21 Display Subtotal void amount        
			  subTotalCash = 0;
	          subTotalCheque = 0;
			  subTotalVoid = 0;
	          officer = false;
        ...
        table.addCell(printStr("Total Cash Collected: HK$ "+totalCash+"0",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Total Cheque Collected: HK$ "+totalCheque+"0",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Total Void Amount: HK$ "+totalVoid+"0",16,12,false,PdfPCell.NO_BORDER,0));    		//2013-02-21 Display Total Void Amount    
		table.addCell(printStr(" ",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Initial of Banking Staff",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr(" ",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Certified By:",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr(" ",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Counter Signed By:",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr(" ",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Amount B/F:",16,12,false,PdfPCell.NO_BORDER,0));
        table.addCell(printStr("Total Amount in Bank for the month:",16,12,false,PdfPCell.NO_BORDER,0));
        float[] widths4 = {15,38,13,13,10,
                              23,10,6};
    ...
    class MyPageEvents extends PdfPageEventHelper { //Override iText's class to handle total page number.
	    ...
	    //subtab1.addCell(printStr("Period End Date: "+reData.getData(reData.periodEnd),1,8,false,PdfPCell.NO_BORDER,0));
          ...
          //sub table 2
            ...
            subtab2.addCell(printStr("TOTAL STAFF TRANSACTIONS REPORT",1,12,true,PdfPCell.NO_BORDER,1));
          ...
          //sub table 3  
            ...
            table.addCell(cel1);
            //col headers
            table.addCell(printStr("Name",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0));        
            table.addCell(printStr("Description",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0));      
            table.addCell(printStr("Cash (HK$)",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            table.addCell(printStr("Cheque (HK$)",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0));       
            table.addCell(printStr("Receipt No",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            table.addCell(printStr("Receipt Date/Time",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            table.addCell(printStr("Officer",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            table.addCell(printStr("Void",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            float[] widths4 = {15,38,13,13,10,
                              23,10,6};
            table.setWidths(widths4);
