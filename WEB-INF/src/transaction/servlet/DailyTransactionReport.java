...
public class DailyTransactionReport extends MasterServlet   {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
	...
	try {
	  ...
	  response.setHeader("Content-Disposition", "attachment; filename=\"DailyTransactionReport.pdf\"");
	  response.setHeader("Cache-control", "no-store, no-cache, must-revalidate");
	  response.setHeader("Pragma", "no-cache");
      ByteArrayOutputStream baos = new ByteArrayOutputStream();
      OutputStream out = response.getOutputStream();     
      JavaReportGenerate rptDrv = new JavaReportGenerate();
      ArrayList data = rptDrv.getDailyTransactionReportData(reData);
      DailyTransactionReportGenerate gen = new DailyTransactionReportGenerate(data, reData);
