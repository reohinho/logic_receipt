...
public class TransactionSummaryReportGenerate  {
	ArrayList data;
	  ReportEnquiryData reData;
	  public TransactionSummaryReportGenerate(ArrayList data, ReportEnquiryData reData) {
	    try {
	      this.data = data;
	      this.reData = reData;
	    }catch(Exception e) {
	...
	public ByteArrayOutputStream getReportCode() { 
		...
		Document document = new Document(PageSize.A4,20,20,110,20); 
		try{
      ...
      if(data.size() == 0) {
        ...
        for(int i=0; i<data.size();i++) {
          ...
          border +=top;
          amt = Double.parseDouble(row[1]);
          table.addCell(printStr(row[0],1,10,false,border,0));        
          table.addCell(printStr(row[2],1,10,false,border,0));                         
          table.addCell(printStr(amt+"0",1,10,false,border,0));                   
         
        } 
        
        float[] widths4 = {30,17,15};
    ...
    class MyPageEvents extends PdfPageEventHelper { //Override iText's class to handle total page number.
        ...
        public void onStartPage(PdfWriter writer, Document document) {
          try {
            ...
            String type = "All";
	    if(reData.getData(reData.receiptType).equals("A"))
	    {
		type = "Visa";
	    }
	    else  if(reData.getData(reData.receiptType).equals("B"))
	    {
		type = "Others";
	    }
	      ...
	      //subtab1.addCell(printStr("Period End Date: "+reData.getData(reData.periodEnd),1,8,false,PdfPCell.NO_BORDER,0));
            subtab1.addCell(printStr("Type: "+type,1,8,false,PdfPCell.NO_BORDER,0));
          ...
          //sub table 2
            ...
            subtab2.addCell(printStr("TRANSACTION SUMMARY REPORT",1,12,true,PdfPCell.NO_BORDER,1));
          ...
          //sub table 3  
            ...
            cel1.setFixedHeight(70f);
            table.addCell(cel1);
            //col headers
            table.addCell(printStr("Item",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0));        
            table.addCell(printStr("Number of Transactions",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0));      
            table.addCell(printStr("Total Amount HK$",1,10,true,PdfPCell.LEFT+PdfPCell.RIGHT+PdfPCell.TOP+PdfPCell.BOTTOM,0)); 
            float[] widths4 = {30,17,15};
            table.setWidths(widths4);
