...
public abstract class AppEditServlet extends BaseServlet implements IEditWorkFlow
{
...
//    }
  ...
  protected void doAppEdit (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
    ...
    if (inCmd.equals(getEditBackCommand())) {  // back button in "Edit" confirmation page
      ...
      if (actionUpdate(wfd)) {
        // if action suceed
System.out.println("Update suceed Message!!");  
...
System.out.println("Update Failure Message!!");        
	wfd.setErrFlag(1);
	wfd.addError(1, "10001", getUpdateFailureMessage(wfd) );
  ...
  /** Optional: "Discard" command with follow up form submit in edit main page */
  protected String getDiscardRedirctCommand() { return kEditDiscardRedirct; } 
  ...
  protected String getCommand (HttpServletRequest request) 
  {
    ...
    if ( inCmd.equals(getEditCommand()) || 
         ...
         inCmd.equals(getDiscardRedirctCommand()) ||
  ...
  {
    String s[] = { getPageIdForEditMain() , getPageIdForEditConfirm (), getPageIdForEditFinal() } ;
    return s;
  }

  // Optional command EditDiscardRedirct implementation 
  protected String getPageIdForEditDiscardRedirct () 
  { return null ; }
  protected java.util.Vector getParamsForDiscardRedirct (GenericWebFormData wfd) 
  { return null; }
  protected void discardFollowUp(HttpServletRequest request, HttpServletResponse response, java.util.Vector preReq) throws ServletException, IOException
  {}
  // end EditDiscardRedirct implementation
  
  // abstract method for child servlet to implement
  protected abstract String getPageIdForEditMain () ;
