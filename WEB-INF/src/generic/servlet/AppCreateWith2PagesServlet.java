/*
  $Source: c:/buffer2cvs/CirculationVCs/WsViewControl/ProjectViewControl/src/com/scmp/circ/generic/servlet/AppCreateWith2PagesServlet.java,v $
  $Author: scmp $
 ...
 */
public abstract class AppCreateWith2PagesServlet extends BaseServlet implements ICreateWorkFlow
{
...
//      forwardTo(svInst.getJSPbyId( getPageIdForAddMain() ), request, response); // goto add main page
    ...
    else if (inCmd.equals(kAddPage2))  // go to page 2, i.e. next button in input page 1
    {
      ...
      if ( (formIsValid == true) && validateRequestParams ( (Object)wfd ) ) {
        forwardTo(getPageIdForAddPage2(), request, response); // goto add confirmation page  
      }
      else {
    ...
    else if (inCmd.equals(kAddSave)) { // save button in "Add" page 2
      String objIdx = getFormParameter(request, kObjectIndex) ;
